...
- preprocessing, dataset assembly, and training code have not yet been ported
- the future preprocessing layer still needs to define how `XYZ` targets are normalized and resampled
- raw episodes now include effective per-episode sample rates, but that information is not yet used downstream

### Recommended Implementation Order
1. finish tightening collection and dataset contracts
2. add raw dataset discovery and preprocessing pipelines
